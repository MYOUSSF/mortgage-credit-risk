...
------------------
  ...
  5. Through-the-cycle (TTC) PD — the same fitted Cox model re-scored with
     its time-varying macro covariates (ur_3m_lag, hpi_change) held at their
     long-run training-sample average, producing an actual second PD
     alongside the point-in-time (PIT) one, for Basel IRB capital use
     (EBA/GL/2017/16 §6.2) as distinct from the PIT PD IFRS 9 provisioning
     uses.
...
-------
  ...
  data/processed/survival_pd_horizons.csv   — per-loan PIT + TTC PD at 12m / 24m / 36m / lifetime
  ...
  data/figures/survival_pit_vs_ttc.png
...
config.configure_logging("survival_analysis.log")
...
# Features available in the PD parquet (numeric only for Cox model — a
# narrower subset than config.PD_FEATURES, which includes categoricals).
...
def build_survival_df(pd_df: pd.DataFrame,
    ...
    # Duration must be at least 1 month.  extract_pd_rows() drops the actual
    # default row itself (leakage prevention for the binary target), so the
    # last retained row for an event=1 loan is exactly one reporting period
    # (1 month) before the true default — add it back so duration reflects
    # true time-to-default rather than time-to-last-observation-before-it.
    # Censored (event=0) loans are unaffected: their last retained row *is*
    # the true censoring time.
    df["duration"] = df["loan_age"].clip(lower=1) + df["event"]
...
def compute_ttc_pds(cox:        CoxPHFitter,
                     oos_surv:   pd.DataFrame,
                     features:   list[str],
                     train_surv: pd.DataFrame,
                     macro_cols: list[str] = ("ur_3m_lag", "hpi_change"),
                     horizons:   list[int] = [12, 24, 36, 60]) -> pd.DataFrame:
    """
    Through-the-cycle (TTC) PD, for Basel IRB capital purposes.

    compute_horizon_pds() is point-in-time (PIT) by construction: it scores
    each loan's survival function using that loan's *actual current* macro
    state (ur_3m_lag, hpi_change), so its output moves with the economic
    cycle. Basel IRB capital wants a PD that is stable through the cycle
    (EBA/GL/2017/16 §6.2) — averaging out the current point in the cycle
    rather than reacting to it.

    Rather than a diagnostic long-run average default rate computed after
    the fact (see 08_calibration.py's LRADR comparison), this produces an
    actual second PD by re-scoring the *same* fitted Cox model with the
    time-varying macro covariates held at their long-run training-sample
    average — i.e. "what would this loan's PD be in a typical point of the
    cycle", holding every other loan characteristic (credit score, CLTV,
    DTI, loan age, etc.) fixed at its current value.
    """
    long_run_avg = {
        c: float(train_surv[c].mean())
        for c in macro_cols if c in train_surv.columns
    }

    avail = [f for f in features if f in oos_surv.columns
             and oos_surv[f].notna().any()]
    df_cox = oos_surv[["loan_seq_num", "duration", "event"] + avail].dropna().copy()

    if len(df_cox) == 0:
        log.warning("  No OOS survival data available for TTC PD calculation.")
        return pd.DataFrame()

    for col, avg in long_run_avg.items():
        if col in df_cox.columns:
            df_cox[col] = avg

    log.info("  Computing TTC (macro-neutral) survival functions for %s OOS loans …",
             f"{len(df_cox):,}")
    log.info("  Long-run macro anchor: %s",
             ", ".join(f"{c}={v:.3f}" for c, v in long_run_avg.items()) or "none available")
    ...
    for h in horizons:
        t_idx = sf.index[sf.index <= h]
        probs = (np.zeros(len(df_cox)) if len(t_idx) == 0
                 else 1 - sf.loc[t_idx[-1]].values)
        results.append(pd.Series(probs, name=f"ttc_pd_{h}m"))

    ttc_df = pd.concat(results, axis=1)
    ttc_df.index = df_cox.index
    ttc_df["loan_seq_num"] = df_cox["loan_seq_num"].values
    return ttc_df


def plot_pit_vs_ttc(horizon_df: pd.DataFrame, horizons: list[int]) -> None:
    """Bar chart: mean PIT vs mean TTC PD at each horizon."""
    pit_cols = [f"pd_{h}m" for h in horizons if f"pd_{h}m" in horizon_df.columns]
    ttc_cols = [f"ttc_pd_{h}m" for h in horizons if f"ttc_pd_{h}m" in horizon_df.columns]
    if not pit_cols or not ttc_cols:
        return
    ...
    fig.suptitle(
        ...
        "PIT: scored on each loan's current macro state   "
        "TTC: macro covariates held at their long-run average",
        fontsize=11, fontweight="bold", color="white"
    )
    ...
    for i, (p, t) in enumerate(zip(pit_means, ttc_means)):
        ax.text(i - width / 2, p, f"{p:.3f}%", ha="center", va="bottom", fontsize=8, color="#E2E8F0")
        ax.text(i + width / 2, t, f"{t:.3f}%", ha="center", va="bottom", fontsize=8, color="#E2E8F0")

    ax.set_xticks(x)
    ax.set_xticklabels([f"{h}m" for h in horizons], fontsize=10)
    ax.set_xlabel("Horizon", fontsize=10)
    ax.set_ylabel("Mean PD (%)", fontsize=10)
    ...
    fig.tight_layout()
    path = FIG_DIR / "survival_pit_vs_ttc.png"
    fig.savefig(path, dpi=150, bbox_inches="tight", facecolor="#0F1117")
    plt.close(fig)
    log.info("  → %s", path)
...
def main() -> None:
    ...
    # ── Load ──────────────────────────────────────────────────────────────
    log.info("")
    log.info("[1/6] Loading processed PD data …")
    ...
    log.info("")
    log.info("[2/6] Building survival datasets (duration + event per loan) …")
    ...
    # ── Kaplan-Meier plots ────────────────────────────────────────────────
    log.info("")
    log.info("[3/6] Kaplan-Meier stratified survival curves …")
    ...
    # ── Cox PH model ──────────────────────────────────────────────────────
    log.info("")
    log.info("[4/6] Fitting Cox Proportional Hazards model …")
    ...
    # ── Multi-horizon PD ──────────────────────────────────────────────────
    log.info("")
    log.info("[5/6] Computing multi-horizon PIT PD (12m / 24m / 36m / 60m) …")
    horizons = [12, 24, 36, 60]
    horizon_df = compute_horizon_pds(cox, oos_surv, COX_FEATURES, horizons=horizons)

    # ── Through-the-cycle PD ─────────────────────────────────────────────
    log.info("")
    log.info("[6/6] Computing through-the-cycle (TTC) PD for Basel IRB capital …")
    ttc_df = compute_ttc_pds(cox, oos_surv, COX_FEATURES, train_surv, horizons=horizons)

    if not horizon_df.empty:
        if not ttc_df.empty:
            horizon_df = horizon_df.merge(
                ttc_df.drop(columns=[c for c in ["duration", "event"] if c in ttc_df.columns]),
    ...
    log.info("")
    log.info("=" * 65)
    log.info("Ch.5 complete — Survival analysis generated (PIT + TTC PD).")
