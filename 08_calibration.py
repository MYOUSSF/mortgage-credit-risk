...
--------
  ...
  6. Long-Run Average PD (LRADR) → TTC cycle adjustment — LRADR isn't just
     compared against the calibrated PIT PD as a diagnostic gap: every
     loan's calibrated PIT PD is logit-shifted by the constant offset that
     moves the *population mean* to LRADR (compute_ttc_pd_via_lradr()),
     producing an actual per-loan TTC PD for Basel IRB capital — the same
     central-tendency adjustment technique used when a model family (here,
     LR/XGBoost) doesn't expose a macro covariate that can be "neutralised"
     the way 06_survival_analysis.py's compute_ttc_pds() does for the Cox
     model.
...
-------
  ...
  data/processed/ttc_calibrated_pd.csv        — per-loan PIT + TTC (LRADR-shifted) PD
...
config.configure_logging("calibration.log")
...
# =============================================================================
# TTC CYCLE ADJUSTMENT  (LRADR anchoring)
# =============================================================================

def compute_ttc_pd_via_lradr(y_true_lradr: np.ndarray,
                              pit_pd:       np.ndarray) -> tuple[np.ndarray, float, float]:
    """
    Cycle-adjust a calibrated PIT PD into a through-the-cycle (TTC) PD via
    long-run average default rate (LRADR) anchoring — Basel II §461 / OCC
    SR 11-7 want a capital PD that doesn't move with the current point in
    the cycle, distinct from the point-in-time PD IFRS 9 provisioning uses.

    Shifts every loan's PIT log-odds by the constant offset that would move
    the population mean PD from its current-period level to the observed
    long-run average default rate (y_true_lradr.mean()) *if* every loan
    shared that mean PIT PD. Because the sigmoid is non-linear, the shifted
    population's actual mean lands close to but not exactly at LRADR for a
    heterogeneous portfolio (Jensen's inequality) — same approximate-anchor
    behaviour as TemperatureCalibrator's bias term above. A pure logit shift
    preserves each loan's rank order while producing an actual per-loan TTC
    PD — not just the diagnostic gap between two aggregate numbers that
    LRADR comparisons (plot_lradr()) report on their own.

        logit(TTC_i) = logit(PIT_i) + [logit(LRADR) - logit(mean(PIT))]

    This is a simpler alternative to 06_survival_analysis.py's
    compute_ttc_pds(), which re-scores the underlying model with macro
    covariates held at their long-run average — that structural approach
    isn't available here since 08_calibration.py only has each model's raw
    score column, not the fitted LR/XGBoost model object itself.

    Returns (ttc_pd, lradr, shift).
    """
    eps = 1e-7
    lradr    = float(np.clip(y_true_lradr.mean(), eps, 1 - eps))
    mean_pit = float(np.clip(pit_pd.mean(), eps, 1 - eps))
    shift    = np.log(lradr / (1 - lradr)) - np.log(mean_pit / (1 - mean_pit))

    pit_clipped = np.clip(pit_pd, eps, 1 - eps)
    logit_pit   = np.log(pit_clipped / (1 - pit_clipped))
    ttc_pd      = 1 / (1 + np.exp(-(logit_pit + shift)))
    return ttc_pd, lradr, shift
...
def main() -> None:
    ...
    log.info("")
    log.info("[1/6] Loading raw model scores …")
    ...
    log.info("")
    log.info("[2/6] Calibrating models …")
    ...
    # ── Comparison chart ───────────────────────────────────────────────────
    log.info("")
    log.info("[4/6] Generating calibration comparison charts …")
    if not metrics_df.empty:
        plot_calibration_comparison(metrics_df)

    # ── LRADR ─────────────────────────────────────────────────────────────
    log.info("")
    log.info("[5/6] Long-Run Average Default Rate comparison …")
    ...
    if models_available:
        ...
        # ── TTC cycle adjustment ───────────────────────────────────────────
        log.info("")
        log.info("[6/6] Cycle-adjusting calibrated PD to a TTC anchor (LRADR shift) …")

        y_true_lradr = np.concatenate([y_true_by_split["oos-eval"], y_true_by_split["oot"]])
        pit_pd_platt = np.concatenate([pd_by_split["oos-eval"]["Platt"], pd_by_split["oot"]["Platt"]])
        ttc_pd, lradr, shift = compute_ttc_pd_via_lradr(y_true_lradr, pit_pd_platt)

        loan_col = "loan_seq_num" if "loan_seq_num" in oos_sub.columns else None
        loan_ids = (
            np.concatenate([oos_sub[loan_col].values, oot_sub[loan_col].values])
            if loan_col else np.arange(len(y_true_lradr))
        )
        split_labels = np.concatenate([
            np.full(len(oos_sub), "oos-eval"), np.full(len(oot_sub), "oot"),
        ])

        ttc_df = pd.DataFrame({
            "loan_seq_num": loan_ids,
            "split":        split_labels,
        ...
        log.info("  Mean PIT (Platt-calibrated) PD: %.4f%%  |  Mean TTC PD: %.4f%%  |  logit shift: %+.4f",
                 pit_pd_platt.mean() * 100, ttc_pd.mean() * 100, shift)
        log.info("  TTC calibrated PD → data/processed/ttc_calibrated_pd.csv")

    log.info("")
    log.info("=" * 65)
    log.info("Ch.7 complete — PD calibration + TTC cycle adjustment generated.")
    log.info("  Next: python 09_monitoring.py")
    log.info("=" * 65)
